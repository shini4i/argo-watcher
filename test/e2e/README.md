...
| `scripts/state-postgres.sh` | flip the release to `STATE_TYPE=postgres` and assert the Postgres-only path: schema migration Job, real deploy loop, task history surviving a pod restart (in-memory loses it), the shared deploy lock (a lock row written by another writer rejects deploys here), and supersession under contention (the app-serialised `SupersedeAndAdd` transaction) |
