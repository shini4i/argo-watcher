...
func TestListFailedSyncResultResources(t *testing.T) {
	...
	assert.Equal(t, expectedResources, listFailedSyncResultResources(&app))
}
...
func TestFormatSyncResultResource(t *testing.T) {
	t.Run("an ordinary resource is reported by its sync status, not its hook phase", func(t *testing.T) {
		...
		assert.Equal(t,
			...
			formatSyncResultResource(models.ApplicationOperationResource{
				Kind:      "Deployment",
...
func TestRolloutFailureHeadline(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			assert.Equal(t, test.expected, rolloutFailureHeadline(&application, ArgoRolloutAppNotSynced, test.waited))
		})
	}
	...
	t.Run("every other failure keeps the legacy headline", func(t *testing.T) {
		...
		assert.Equal(t,
			...
			rolloutFailureHeadline(&application, ArgoRolloutAppNotAvailable, 225*time.Second))
	})
}
...
func TestAutoSyncEnabled(t *testing.T) {
	...
	tests := []struct {
		name     string
		policy   *models.ApplicationSyncPolicy
		expected bool
	}{
		{"no policy at all", nil, false},
		{"policy without an automated block", &models.ApplicationSyncPolicy{}, false},
		{"automated block", &models.ApplicationSyncPolicy{Automated: &models.ApplicationSyncPolicyAutomated{}}, true},
		{"automated block explicitly enabled", &models.ApplicationSyncPolicy{Automated: &models.ApplicationSyncPolicyAutomated{Enabled: &enabled}}, true},
		{"automated block explicitly disabled", &models.ApplicationSyncPolicy{Automated: &models.ApplicationSyncPolicyAutomated{Enabled: &disabled}}, false},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			assert.Equal(t, test.expected, autoSyncEnabled(&application))
		})
	}
}
...
func TestListUnhealthyResources(t *testing.T) {
	...
	unhealthyResources := listUnhealthyResources(&app)
...
func TestNotSyncedDiagnosticsFromPayload(t *testing.T) {
	...
	// revisions and spec.syncPolicy — are exercised. Tests that build the models.Application in Go
	...
	assert.Equal(t, ArgoRolloutAppNotSynced,
		rolloutStatus(&app, []string{"product-catalog:v0.0.2"}, "", false))
	...
	assert.Equal(t,
		...
		rolloutMessage(&app, ArgoRolloutAppNotSynced, []string{"product-catalog:v0.0.2"}, nil))
}
...
func TestNotSyncedDiagnosticsFromMultiSourcePayload(t *testing.T) {
	...
	// multi-source app depends on (status.sync.revisions, syncResult.revisions,
	// spec.syncPolicy.automated.enabled): a wrong tag fails silently, dropping the drift
	// explanation or reporting an app with automated sync off as one ArgoCD is about to fix.
...
func TestArgoRolloutStatus(t *testing.T) {
	t.Run("Rollout status - ArgoRolloutAppNotAvailable", func(t *testing.T) {
		...
		registryProxyUrl := ""
		assert.Equal(t, ArgoRolloutAppNotAvailable, rolloutStatus(&application, images, registryProxyUrl, false))
	})
	...
	t.Run("Rollout status - ArgoRolloutAppNotSynced", func(t *testing.T) {
		...
		registryProxyUrl := ""
		assert.Equal(t, ArgoRolloutAppNotSynced, rolloutStatus(&application, images, registryProxyUrl, false))
	})
	...
	t.Run("Rollout status - ArgoRolloutAppNotHealthy", func(t *testing.T) {
		...
		images := []string{"ghcr.io/shini4i/argo-watcher:version1"}
		registryProxyUrl := ""
		assert.Equal(t, ArgoRolloutAppNotHealthy, rolloutStatus(&application, images, registryProxyUrl, false))
	})
	...
	t.Run("Rollout status - ArgoRolloutAppSuccess", func(t *testing.T) {
		...
		images := []string{"ghcr.io/shini4i/argo-watcher:version1"}
		registryProxyUrl := ""
		assert.Equal(t, ArgoRolloutAppSuccess, rolloutStatus(&application, images, registryProxyUrl, false))
	})
	...
	t.Run("Rollout status - ArgoRolloutAppDegraded", func(t *testing.T) {
		...
		images := []string{"ghcr.io/shini4i/argo-watcher:version1"}
		registryProxyUrl := ""
		assert.Equal(t, ArgoRolloutAppDegraded, rolloutStatus(&application, images, registryProxyUrl, false))
	})
	...
	t.Run("Rollout status - a degraded app that is still OutOfSync is not synced", func(t *testing.T) {
		...
		application.Status.Health.Status = "Degraded"
		images := []string{"ghcr.io/shini4i/argo-watcher:version1"}
		assert.Equal(t, ArgoRolloutAppNotSynced, rolloutStatus(&application, images, "", false))
	})
	...
	t.Run("acceptSuspended is true", func(t *testing.T) {
		...
		status := rolloutStatus(&application, []string{}, "", true)
	...
	t.Run("acceptSuspended is false", func(t *testing.T) {
		...
		status := rolloutStatus(&application, []string{}, "", false)
		if status == ArgoRolloutAppSuccess {
...
func TestArgoRolloutMessage(t *testing.T) {
	...
	t.Run("Rollout message - ArgoRolloutAppNotAvailable", func(t *testing.T) {
		...
		assert.Equal(t,
			...
			rolloutMessage(&application, ArgoRolloutAppNotAvailable, images, nil))
	})
	...
	t.Run("Rollout message - ArgoRolloutAppNotAvailable with failed hook", func(t *testing.T) {
		...
		application.Status.OperationState.SyncResult.Resources = []models.ApplicationOperationResource{
			{
				HookPhase: "Failed",
				HookType:  "PreSync",
				Kind:      "Job",
	...
	t.Run("Rollout message - ArgoRolloutAppNotAvailable with unhealthy pod", func(t *testing.T) {
		application := models.Application{}
		application.Status.Summary.Images = []string{"app:v0.0.1"}
		application.Status.Resources = []models.ApplicationResource{
			{
				Kind:      "Pod",
				Name:      "app-pod",
				Namespace: "app",
	...
	t.Run("Rollout message - ArgoRolloutAppNotAvailable filters out healthy/successful resources", func(t *testing.T) {
		// Successful hooks, healthy/empty-status pods and a non-failed sync phase must NOT appear in
		// the report. Both `Running` (mid-rollout) and `Succeeded` (steady state) are probed to pin
		// the filter to {Failed, Error} only.
	...
	t.Run("Rollout message - ArgoRolloutAppNotSynced", func(t *testing.T) {
		...
		application.Status.OperationState.SyncResult.Resources = make([]models.ApplicationOperationResource, 2)
		...
		assert.Equal(t,
			...
			rolloutMessage(&application, ArgoRolloutAppNotSynced, images, nil))
	})
	...
	t.Run("Rollout message - ArgoRolloutAppNotHealthy", func(t *testing.T) {
		...
		application.Status.OperationState.Phase = "NotWorking"
		application.Status.OperationState.Message = "Not working test app"
		application.Status.Resources = make([]models.ApplicationResource, 2)
		...
		assert.Equal(t,
			...
			rolloutMessage(&application, ArgoRolloutAppNotHealthy, images, nil))
	})
	...
	t.Run("Rollout message - default", func(t *testing.T) {
		...
		application.Status.Summary.Images = []string{"ghcr.io/shini4i/argo-watcher:version1"}
		images := []string{"ghcr.io/shini4i/argo-watcher:version2"}
		assert.Equal(t, "received unexpected rollout status \"unexpected status\"", rolloutMessage(&application, "unexpected status", images, nil))
	})

	t.Run("Rollout message - ArgoRolloutAppNotAvailable with failed sync operation and empty message", func(t *testing.T) {
		// Pins the OperationState.Message == "" branch: the "Sync operation message:" line must be
		// omitted when ArgoCD provides only a phase.
		application := models.Application{}
		application.Status.Summary.Images = []string{"app:v0.0.1"}
	...
	t.Run("Rollout message - ArgoRolloutAppNotAvailable surfaces pod cause from resource tree", func(t *testing.T) {
		...
		assert.Equal(t,
			...
			rolloutMessage(&application, ArgoRolloutAppNotAvailable, images, tree))
	})
	...
	t.Run("Rollout message - ArgoRolloutAppNotHealthy appends pod cause from resource tree", func(t *testing.T) {
		...
		assert.Equal(t,
			"App sync status \"Synced\"\n"+
				...
				"Unhealthy resources:\n"+
				"\tPod(app-xyz) Degraded with message Back-off pulling image \"app:v0.0.2\": ErrImagePull",
			rolloutMessage(&application, ArgoRolloutAppNotHealthy, images, tree))
	})
	...
	t.Run("Rollout message - ArgoRolloutAppDegraded reports the degraded resource and its pod cause", func(t *testing.T) {
		...
		assert.Equal(t,
			...
			rolloutMessage(&application, ArgoRolloutAppDegraded, images, tree))
	})
	...
	t.Run("Rollout message - not synced closes with the last sync operation", func(t *testing.T) {
		...
		assert.Equal(t,
			"Last sync operation: NotWorking, message: \"Not working test app\"",
			rolloutMessage(&application, ArgoRolloutAppNotSynced, []string{"app:v0.0.2"}, nil))
	})
	...
	t.Run("Rollout message - not synced explains a superseded revision", func(t *testing.T) {
		// The reported confusion: the sync operation succeeded while the app stayed out of sync,
		// which reads as a contradiction until the report says the target moved underneath it. The
		// wording claims no ordering between the revisions on purpose: a rollback, revert or
		// retargeted branch leaves the comparison at an older revision, so "newer" would mislead.
	...
	t.Run("Rollout message - not synced calls out drift that will never converge", func(t *testing.T) {
		...
		application.Spec.SyncPolicy = &models.ApplicationSyncPolicy{Automated: &models.ApplicationSyncPolicyAutomated{SelfHeal: true}}
		assert.Equal(t,
	...
	t.Run("Rollout message - not synced reports the pod-level cause when the app is also degraded", func(t *testing.T) {
		...
		assert.Equal(t,
			"Unhealthy resources:\n"+
				...
				"\tDeployment(product-catalog) OutOfSync\n\n"+
				"Last sync operation: Succeeded, message: \"successfully synced (all tasks run)\"",
			rolloutMessage(&application, ArgoRolloutAppNotSynced, []string{"app:v0.0.2"}, tree))
	})
	...
	t.Run("Rollout message - not synced omits the Sync errors block when only warnings are present", func(t *testing.T) {
		...
		application := notSyncedApp("Succeeded", "successfully synced (all tasks run)", "", "")
		application.Status.Conditions = []models.ApplicationCondition{
	...
	t.Run("Rollout message - not synced orders every section of a full report", func(t *testing.T) {
		...
		application := notSyncedApp("Succeeded", "successfully synced (all tasks run)",
			"a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0", "0f9e8d7c6b5a4f3e2d1c0b9a8f7e6d5c4b3a2f10")
		application.Spec.SyncPolicy = &models.ApplicationSyncPolicy{Automated: &models.ApplicationSyncPolicyAutomated{Prune: true}}
		application.Status.Conditions = []models.ApplicationCondition{
			{Type: "SyncError", Message: "Failed sync attempt to v1.2.3"},
		}
		application.Status.Resources = []models.ApplicationResource{
	...
	t.Run("Rollout message - a degraded app suppresses the still-progressing fallback", func(t *testing.T) {
		...
		images := []string{"app:v0.0.2"}
		msg := rolloutMessage(&application, ArgoRolloutAppDegraded, images, tree)
	...
	t.Run("Rollout message - a problem resource that is not the culprit still reports what is progressing", func(t *testing.T) {
		...
		msg := rolloutMessage(&application, ArgoRolloutAppNotHealthy, []string{"app:v0.0.2"}, tree)
...
func notSyncedApp(phase, message, applied, compared string) models.Application {
	application := models.Application{}
	application.Status.Sync.Status = "OutOfSync"
...
func treeNode(kind, name, status, message string) models.ApplicationTreeNode {
	n := models.ApplicationTreeNode{Kind: kind, Name: name, Namespace: "app"}
...
func TestIsTerminalFailurePhase(t *testing.T) {
	// Pins the {Failed, Error} predicate behaviourally: the rollout-message tests cover it only
	// structurally, so dropping "Error" would fail no other test.
	tests := []struct {
		phase string
		want  bool
	}{
...
func TestIsProblemHealthStatus(t *testing.T) {
	// Pins the {Degraded, Missing, Unknown, Suspended} predicate behaviourally: the rollout-message
	// tests cover only "Degraded", so narrowing the filter would fail no other test.
	tests := []struct {
		status string
		want   bool
	}{
