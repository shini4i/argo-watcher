...
type ServerConfig struct {
	...
	KeycloakUrl                     string   `env:"KEYCLOAK_URL" json:"keycloak_url,omitempty"` // setting this enables Keycloak integration
	...
	KeycloakTokenValidationInterval int      `env:"KEYCLOAK_TOKEN_VALIDATION_INTERVAL" envDefault:"10000" json:"keycloak_token_validation_interval"` // how often to check if Keycloak token is still valid in Milliseconds
	KeycloakPrivilegedGroups        []string `env:"KEYCLOAK_PRIVILEGED_GROUPS" json:"keycloak_privileged_groups,omitempty"`                          // members of these groups will be able to use rollback button in UI
}
