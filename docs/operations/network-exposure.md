...
- **Encrypt the database hop.** With `STATE_TYPE=postgres` the connection carries the whole deployment history, the deploy lock, task leases and the application-token rows. By default the driver attempts TLS but accepts any certificate, and falls back to cleartext if the server refuses TLS outright — neither path detects an attacker on the path. `require` does not check the certificate either. Only `verify-ca` and `verify-full` do, so set [`DB_SSL_MODE=verify-full`](../reference/server-env.md#database) with `PGSSLROOTCERT` whenever the database is not reached over a loopback interface or a Unix socket.
